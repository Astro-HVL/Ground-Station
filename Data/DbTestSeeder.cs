...
namespace TelemetryWebApp.Data
{
    ...
    public class DbTestSeeder
    {
        ...
        private static async Task<int> EnsureRocketAsync(NpgsqlConnection conn, string name)
        {
            ...
            const string insert = @"INSERT INTO rocket (name, serial_number, notes)
                                VALUES (@name, 'SN-TEST', 'Seeded test rocket')
                                RETURNING rocket_id;";
        ...
        private static async Task<int> EnsureFlightAsync(NpgsqlConnection conn, int rocketId, string missionName)
        {
            ...
            const string insert = @"INSERT INTO flight (rocket_id, mission_name, launch_site, planned_launch_at, launch_at, notes)
                                VALUES (@rid, @mission, 'Test Range', now(), now(), 'Seeded test flight')
                                RETURNING flight_id;";
            await using var cmdInsert = new NpgsqlCommand(insert, conn);
        ...
        private static async Task InsertTestTelemetryAsync(NpgsqlConnection conn, int flightId, int rows, int dtMs)
        {
            ...
            INSERT INTO telemetry (
                flight_id, seq, t_ms, received_at,
                ax, ay, az, pitch_deg, roll_deg, yaw_deg,
                temperature_c, velocity_mps, pressure_pa,
                latitude_deg, longitude_deg, altitude_m, state_id, raw_line
            )
            VALUES (
                @flight_id, @seq, @t_ms, now(),
                @ax, @ay, @az, @pitch, @roll, @yaw,
                @temp, @vel, @press,
                @lat, @lon, @alt, @state, @raw
            );";
            ...
            cmd.Parameters.Add("seq", NpgsqlTypes.NpgsqlDbType.Integer);
            ...
            cmd.Parameters.Add("raw", NpgsqlTypes.NpgsqlDbType.Text);
            ...
            for (int i = 0; i < rows; i++, tMs += dtMs)
            {
                ...
                // Recreate the same CSV line format as the rocket firmware output
                string raw = string.Join(",", tMs.ToString(inv), i, ax, ay, az, pitch, roll, yaw,
                                         temp, vel, press, lat, lon, alt, state);
